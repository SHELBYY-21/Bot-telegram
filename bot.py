...
THB_AMOUNT_RE = re.compile(
    r"^([0-9]+(?:\.[0-9]+)?)\s*(?:thb|฿|baht)$", re.I
)
USDT_DECIMAL_RE = re.compile(r"^([0-9]+\.\d{3,8})$")


def parse_amount(text: str) -> tuple[str, float] | None:
    """Return ('usdt'|'thb', value). USDT decimals (3+) or explicit suffix."""
    m = USDT_AMOUNT_RE.match(text)
    if m:
        return "usdt", float(m.group(1))
    m = re.match(r"^usdt\s+([0-9]+(?:\.[0-9]+)?)$", text, re.I)
    if m:
        return "usdt", float(m.group(1))
    m = THB_AMOUNT_RE.match(text)
    if m:
        return "thb", float(m.group(1))
    m = USDT_DECIMAL_RE.match(text)
    if m:
        return "usdt", float(m.group(1))
    # Plain integer or 2-decimal amount defaults to THB (desk context).
    m = re.match(r"^([0-9]+(?:\.[0-9]{1,2})?)$", text)
    if m:
        return "thb", float(m.group(1))
    return None
...
async def begin_from_thb(
    update: Update, context: ContextTypes.DEFAULT_TYPE, thb_amount: float
) -> None:
    user = update.effective_user
    assert user and update.effective_chat
    ...
    await show_loading(update, context, "Calculating", 40)
    q = quote(context)
    amounts = compute_from_thb(thb_amount, q)
    store = ledger(context)
    entry = store.create_entry(
        status=Status.RECEIVED.value,
        thb=amounts["thb"],
...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Amount shortcut (USDT or THB)
    parsed = parse_amount(text)
    if parsed:
        kind, amount = parsed
        context.user_data.pop("console_message_id", None)
        if kind == "usdt":
            await begin_from_usdt(update, context, amount)
        else:
            await begin_from_thb(update, context, amount)
        return
